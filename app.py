...
def init_db():
    ...
    # Safely alter table to add user_id column if database existed before
    try:
...
def login_user(email, password):
    ...
    return True, {"id": user["id"], "email": email}

# Database Functions
def save_calculation(transport, energy, diet, waste, total, user_id=None):
    conn = sqlite3.connect(DB_FILE, timeout=30.0)
    cursor = conn.cursor()
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (timestamp, transport, energy, diet, waste, total, user_id))
    conn.commit()
    conn.close()
...
# Initialize session state for theme and user
...
with st.sidebar:
    ...
    if st.session_state.user:
        ...
        auth_action = st.radio("Choose Action", ["Sign In", "Register"], label_visibility="collapsed")
...
if not st.session_state.user:
    ...
    st.markdown(
        ...
        f'👤 <strong>Persistent Session Active:</strong> Syncing calculations for <strong>{st.session_state.user["email"]}</strong>.'
        '</div>',
        unsafe_allow_html=True
    )
        ...
        with st.container(border=True):
            ...
            if st.button("Commit to History Ledger", use_container_width=True, key="save_btn", type="primary"):
                if raw_total > 0:
                    ...
                    save_calculation(sim_trans, sim_energ, sim_diet, sim_waste, projected_total, user_id)
